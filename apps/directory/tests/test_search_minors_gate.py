...
from apps.directory.services import search, verification
...
def test_both_halves_of_the_gate_are_present_in_the_source():
    ...
    app = Path(__file__).resolve().parents[3] / "apps" / "directory"
    assert app.is_dir(), f"{app} has moved — this grep is watching nothing"

    search_source = (app / "services" / "search.py").read_text()
    assert "minor_work_status=MinorWorkStatus.CLEARED" in search_source, (
        "the search-queryset half of the under-18 gate has gone"
    )

    model_source = (app / "models.py").read_text()
    assert "def can_show_minor_groups" in model_source
    assert "def visible_client_groups" in model_source

    profile_source = (app / "services" / "profile.py").read_text()
...
def child_speciality():
    from apps.directory.factories import SpecialityFactory
